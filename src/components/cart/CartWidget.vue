<template>
  <button class="p-link layout-topbar-button" :title="cartTitle" @click="changeVisibility">
    <i class="pi pi-shopping-cart" :class="{cartColor: isItemsInCart}"></i>
    <span title="Items" :class="{cartColor: isItemsInCart}" style="display: block; margin-left: 0.3rem;">{{
        itemsCount
      }}</span>
  </button>
  <div class="nice-cart" v-show="getCartVisibility">
...
export default {
  ...
  methods: {
    ...
    async getDiscounts() {
      ...
      this.setDiscountsList(JSON.parse(window.localStorage.getItem('discounts')));
    },
