<template>
  <div class="p-paginator p-component" v-if="alwaysShow ? true : (pageLinks && pageLinks.length > 1)">
    ...
    <template v-for="item of templateItems" :key="item">
      <first-page-link v-if="item === 'FirstPageLink'" @click="changePageToFirst($event)"
                       :disabled="isFirstPage || empty"></first-page-link>
      <PrevPageLink v-else-if="item === 'PrevPageLink'" @click="changePageToPrev($event)"
                    :disabled="isFirstPage || empty"/>
      <NextPageLink v-else-if="item === 'NextPageLink'" @click="changePageToNext($event)"
                    :disabled="isLastPage || empty"/>
      <LastPageLink v-else-if="item === 'LastPageLink'" @click="changePageToLast($event)"
                    :disabled="isLastPage || empty"/>
      <PageLinks v-else-if="item === 'PageLinks'" :value="pageLinks" :page="page" @click="changePageLink($event)"/>
      <current-page-report v-else-if="item === 'CurrentPageReport'" :template="currentPageReportTemplate"
                         :currentPage="currentPage"
                         :page="page" :pageCount="pageCount" :first="d_first" :rows="d_rows"
                           :totalRecords="totalRecords"></current-page-report>
      ...
      <JumpToPageInput v-else-if="item === 'JumpToPageInput'" :page="currentPage" @page-change="changePage($event)"
                       :disabled="empty"/>
    </template>
